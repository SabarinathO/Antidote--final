...
function confirmDelivery(radio) {
    ...
    const statusCustomer = orderCard.querySelector(".status-customer"); // Customer name span

    if (confirm("Are you sure you have sent this item?")) {
        ...
        const customerName = orderCard.getAttribute("data-customer"); // Get customer name
        statusCustomer.textContent = `Customer: ${customerName}`; // Display customer name
...
function confirmNotReceived(radio) {
  ...
  if (confirm("Are you sure you have not sent this item?")) {
